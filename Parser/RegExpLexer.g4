...
STARTCODEANCHOR 
	: '{'	{
				if(_input.La(-2)!=92) { Mode(ACTIONCODE); }
...
CLOSURE_FLAG 
	: { guard }? { LBindex =InputStream.Index; } '~{' INTEGER ( ',' INTEGER? )? '}'	{
																						RegExpLexer.guard = false;
...
STRING_FLAG 
	: { guard_string }? '\'' {LBindex =InputStream.Index; } .*? '\''  	{
...
RAC
	: '}'	{
				RegExpLexer.guard = true;
				Mode(RegExpLexer.REGEXP);
				Console.WriteLine("Enter RegExp Mode");
			}
;
...
STRING
	:  .*? '\''	{
					RegExpLexer.guard_string = true;
					if ( _token !=null ) Console.WriteLine(_token.Text);
				}->mode(REGEXP)
;
